...
version 1.0.0 (2026-07-06) — 正本。実装とズレたらコードを直す。仕様変更は version bump + changelog。
...
- **R2**: Generate 実行時、システムは Style(8種) / Complexity / Fill / Bars(4·8·16) / Key / Mode に基づき、JS 版と同じアルゴリズム（パターン選択 → Markov 遷移 → 変奏 → 後半のみフレーズ展開）でベースラインを生成する
